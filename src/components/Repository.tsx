...
function Repository(props: { data: Repo }) {
  ...
  useEffect(() => {
    if (props.data.topics !== undefined) {
      const topicos = props.data.topics
        ?.filter((topic: string) => topicClasses[topic])
        .map((topic: string) => topicClasses[topic]);
    ...
    <div className="md:flex md:space-x-4">
      <div className="min-w-[120px] space-y-2 flex justify-between w-full md:max-w-[120px] md:block">
        <div className="space-x-2">
          {props.data.fork ? (
            <p className="text-zinc-700 dark:text-zinc-400">
              <i className="nf nf-cod-repo_forked"></i>{" "}
              <span className="share">fork</span>
            </p>
          ) : (
        ...
        <div className="flex items-center space-x-2 text-sm">
          <i className="nf nf-cod-repo_forked text-zinc-700 dark:text-zinc-400">
            {" "}
            {props.data.forks}
          </i>
          <i className="nf nf-md-star text-zinc-700 dark:text-zinc-400">
            {" "}
      ...
      <div>
        <h1 className="capitalize text-lg">
          <a
            ...
            className="hover:text-teal-700 transition-all ease-linear duration-200"
            target="_blank"
          >
            {props.data.name} <span className="nf nf-oct-arrow_up_right"></span>
          </a>
        </h1>
        {license && <div className="text-zinc-600">{license?.spdx_id}</div>}
        <div className="text-zinc-700 dark:text-zinc-400 min-h-[100px] text-sm pb-8 flex-grow">
    ...
    const getRepos = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/users/ldcmleo/repos",
          {
            headers: {
        ...
        const final_data = data.filter((repo: Repo) =>
          IncludedRepositories.includes(repo.name),
        );
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        ...
        repos.map((repo: Repo) => <Repository key={repo.name} data={repo} />)
      )}
    </div>
  );
}
